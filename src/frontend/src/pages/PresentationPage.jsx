...
function SlideHero() {
  return (
    <div style={{ textAlign: 'center', maxWidth: 760, margin: '0 auto' }}>
      <div className="sa sa-1" style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 30 }}>
      ...
      <div className="sa sa-2"><div className="pt-label">M1 Demo · June 2026</div></div>
      ...
      <div className="sa sa-4">
        <p className="pt-body" style={{ maxWidth: 560, margin: '0 auto 32px' }}>
      ...
      <div className="sa sa-5" style={{ display: 'flex', gap: 10, justifyContent: 'center', alignItems: 'center' }}>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,.28)', letterSpacing: '.1em', textTransform: 'uppercase' }}>Press → to begin</span>
        <span className="arrow-hint" style={{ fontSize: 18, color: 'rgba(255,255,255,.3)' }}>→</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div className="pg-card sa sa-3" style={{ borderColor: 'rgba(239,68,68,.2)' }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: '#ef4444', marginBottom: 14 }}>✕  Before</div>
          {[
            ...
            <div key={t} style={{ marginBottom: 13 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(239,68,68,.7)', marginBottom: 3 }}>{t}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.3)', lineHeight: 1.45 }}>{d}</div>
            </div>
          ))}
        </div>
        <div className="pg-card sa sa-4" style={{ borderColor: 'rgba(79,142,255,.25)' }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: '#4f8eff', marginBottom: 14 }}>✓  After</div>
          {[
            ...
            <div key={t} style={{ marginBottom: 13 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#4f8eff', marginBottom: 3 }}>{t}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.3)', lineHeight: 1.45 }}>{d}</div>
            </div>
          ))}
        </div>
    ...
    <div>
      ...
      <div className="sa sa-3">
        <svg viewBox="0 0 780 285" width="100%" style={{ maxHeight: 285, display: 'block' }}>
          ...
          {/* Browser node */}
          <rect x="290" y="8" width="200" height="54" rx="12" fill="rgba(255,255,255,.05)" stroke="rgba(255,255,255,.12)" strokeWidth="1" />
          <text x="390" y="30" textAnchor="middle" fill="rgba(255,255,255,.75)" fontSize="11" fontWeight="700" fontFamily="Inter,system-ui">Browser · React UI</text>
          <text x="390" y="49" textAnchor="middle" fill="rgba(255,255,255,.3)" fontSize="10" fontFamily="Inter,system-ui">Vite · Port 5173</text>

          {/* API node */}
          <rect x="274" y="115" width="232" height="54" rx="12" fill="rgba(79,142,255,.1)" stroke="rgba(79,142,255,.32)" strokeWidth="1.5" />
          <text x="390" y="137" textAnchor="middle" fill="#4f8eff" fontSize="11" fontWeight="700" fontFamily="Inter,system-ui">Node.js · Express API</text>
          <text x="390" y="155" textAnchor="middle" fill="rgba(79,142,255,.55)" fontSize="10" fontFamily="Inter,system-ui">REST · Bearer Auth · Port 3001</text>

          {/* Provider nodes */}
          <rect x="70"  y="248" width="185" height="36" rx="10" fill="rgba(255,153,0,.08)"  stroke="rgba(255,153,0,.25)"  strokeWidth="1" />
          <text x="163" y="270" textAnchor="middle" fill="#ff9900" fontSize="12" fontWeight="700" fontFamily="Inter,system-ui">AWS S3</text>
          <rect x="297" y="248" width="185" height="36" rx="10" fill="rgba(79,159,245,.08)" stroke="rgba(79,159,245,.25)" strokeWidth="1" />
          <text x="390" y="270" textAnchor="middle" fill="#4f9ff5" fontSize="12" fontWeight="700" fontFamily="Inter,system-ui">Azure Blob</text>
          <rect x="525" y="248" width="185" height="36" rx="10" fill="rgba(52,168,83,.08)"  stroke="rgba(52,168,83,.25)"  strokeWidth="1" />
          <text x="618" y="270" textAnchor="middle" fill="#34c674" fontSize="12" fontWeight="700" fontFamily="Inter,system-ui">GCS</text>

          {/* Static dashed connectors */}
          <line x1="390" y1="62" x2="390" y2="115" stroke="rgba(255,255,255,.12)" strokeWidth="1.5" strokeDasharray="4 3" />
          <path d="M350 169 Q195 205 162 248" fill="none" stroke="rgba(255,153,0,.18)"  strokeWidth="1.5" strokeDasharray="4 3" />
          <line x1="390" y1="169" x2="390" y2="248"                                     stroke="rgba(79,159,245,.18)" strokeWidth="1.5" strokeDasharray="4 3" />
          <path d="M430 169 Q590 205 618 248" fill="none" stroke="rgba(52,168,83,.18)"  strokeWidth="1.5" strokeDasharray="4 3" />
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 14 }}>
        ...
        <div className="pg-card sa sa-3" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,.3)', textTransform: 'uppercase', letterSpacing: '.12em' }}>Total Files</div>
          <div className="pres-num">{loading ? '—' : <CountUp to={totalFiles} />}</div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,.25)' }}>across all providers</div>
        </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 11 }}>
        {cards.map((c, i) => (
          <div key={c.title} className={`pg-card sa sa-${Math.min(i + 2, 6)}`} style={{ padding: '16px', borderColor: c.color + '30' }}>
            <div style={{ width: 38, height: 38, borderRadius: 11, background: c.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, marginBottom: 10 }}>{c.icon}</div>
            <div style={{ fontSize: 12.5, fontWeight: 700, color: '#fff', marginBottom: 5 }}>{c.title}</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,.38)', lineHeight: 1.5 }}>{c.desc}</div>
          </div>
        ))}
      </div>
    </div>
...
function SlideClosing() {
  ...
  const milestones = [
    { id: 'M1', label: 'API Integration',  date: 'Gate Jun 5',  active: true,  items: ['AWS · Azure · GCS live', 'React UI · Auth · RBAC', 'Direct upload on Vercel'] },
    { id: 'M2', label: 'Hardening',        date: 'Gate Jun 12', active: false, items: ['Unit + integration tests', 'P0/P1 code-review fixes', 'E2E production validation'] },
    { id: 'M3', label: 'Production',       date: 'Gate Jun 15', active: false, items: ['Persistent audit log', 'JWT RBAC UI', 'Performance tuning'] },
  ];
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 28, alignItems: 'start' }}>
      <div>
        ...
        <div className="sa sa-2"><h2 className="pt-h2" style={{ marginBottom: 20 }}>20 days to go-live</h2></div>

        {milestones.map((m, i) => (
          <div key={m.id} className={`sa sa-${i + 3}`} style={{ display: 'flex', gap: 16, marginBottom: 14 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: m.active ? '#10b981' : 'rgba(255,255,255,.07)', border: `2px solid ${m.active ? '#10b981' : 'rgba(255,255,255,.14)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: m.active ? '#fff' : 'rgba(255,255,255,.28)', fontSize: 11, fontWeight: 800 }}>
                {m.active ? '✓' : m.id[1]}
              </div>
              {i < 2 && <div style={{ width: 1, height: 36, background: 'rgba(255,255,255,.07)', marginTop: 4 }} />}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: m.active ? '#10b981' : '#fff' }}>{m.id} · {m.label}</span>
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,.28)', fontWeight: 600 }}>{m.date}</span>
                {m.active && <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 999, background: 'rgba(16,185,129,.15)', color: '#10b981', border: '1px solid rgba(16,185,129,.28)' }}>IN PROGRESS</span>}
              </div>
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {m.items.map(it => (
                  <span key={it} style={{ fontSize: 10.5, color: 'rgba(255,255,255,.32)', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 6, padding: '2px 8px' }}>{it}</span>
                ))}
              </div>
            </div>
          </div>
      ...
      <div>
        <div className="pg-card sa sa-2" style={{ textAlign: 'center', marginTop: 52 }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.28)', marginBottom: 14 }}>Time to Go-Live</div>
          <Countdown target={GO_LIVE} />
          <div className="grad-sep" />
          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#fff', marginBottom: 2 }}>June 16, 2026</div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,.28)' }}>Azure App Service F1 · Production</div>
          <div className="grad-sep" />
  ...
  return (
    <div className="pres">
      ...
      <div className="pres-top-bar">
        <div className="pres-brand">Multi-Cloud Storage · M1 Demo</div>
